(() => {
  ...
  const CLUE_IMAGES = {
    papers: { src: "assets/crop1_papers.png", alt: "机の上の書類" },
    whiteboard: { src: "assets/crop2_whiteboard.png", alt: "ホワイトボードの走り書き" },
    panel: { src: "assets/crop3_panel.png", alt: "実験装置の表示パネル" },
  };
  ...
  // （答えを平文でこのファイルに書かないこと）
  const TARGET_HASH = 2088261201; // simpleHash("0804")

  const discovered = { papers: false, whiteboard: false, panel: false };
  ...
  function maybeRevealCodeSection() {
    if (discovered.papers && discovered.whiteboard && discovered.panel) {
  ...
  //             何か書いてあった気がする。」
  const HINT_TEXT = "書類に書かれた番号は、本当にそのままの数字だろうか——ホワイトボードにも、何か書いてあった気がする。";
  ...
  function unlockDrawer() {
    ...
    playItemEffect(item.style.left, item.style.top, "#8fe3f0");
  }
